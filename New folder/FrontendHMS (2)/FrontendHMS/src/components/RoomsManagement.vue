...
const applyFilter = async () => {
  ...
  // If we have a status filter, use the status-specific endpoint
  if (selectedStatusFilter.value) {
    try {
      await roomsStore.fetchRoomsByStatus(selectedStatusFilter.value);
      
      // If we also have a room type filter, apply it client-side
      if (roomTypeId) {
      ...
      // Fall back to regular room fetching with client-side filtering
      await roomsStore.fetchRooms(1, 100, roomTypeId);
    }
  } else {
    // No status filter, just fetch by room type if needed
    await roomsStore.fetchRooms(1, 100, roomTypeId);
  }
}
