...
// Computed property to ensure we always have status counts
const computedStatusCounts = computed(() => {
  return {
    available: roomsStore.statusCounts.available || 0,
    occupied: roomsStore.statusCounts.occupied || 0,
    cleaning: roomsStore.statusCounts.cleaning || 0,
    maintenance: roomsStore.statusCounts.maintenance || 0,
    out_of_order: roomsStore.statusCounts.out_of_order || 0
  }
})

// Update our status counts whenever the computed value changes
watch(computedStatusCounts, (newCounts: {
  available: number;
  occupied: number;
  cleaning: number;
  maintenance: number;
  ...
  statusCounts.value = newCounts
})

// Handle room status updates
const handleRoomStatusUpdate = async () => {
  // Refresh status counts from API when room status changes
  try {
    const apiStatusCounts = await roomsStore.fetchAllStatusCounts()
    statusCounts.value = apiStatusCounts
  } catch (error) {
    console.error('Failed to refresh status counts from API, falling back to store calculation:', error)
    // Fallback: fetch all rooms and calculate stats client-side
    try {
      await roomsStore.fetchRooms(1, 200) // Fetch more rooms for better calculation
      statusCounts.value = computedStatusCounts.value
    } catch (fetchError) {
      console.error('Failed to fetch rooms for status calculation:', fetchError)
    }
  }
}
...
onMounted(async () => {
  ...
  if (!authStore.isAuthenticated || !authStore.isEmployee) {
    console.error('User is not authenticated or does not have employee role');
    return;
  }
  ...
  await Promise.all([
    ...
    // Fetch and set actual status counts from API (for all rooms, not just current page)
  try {
    const apiStatusCounts = await roomsStore.fetchAllStatusCounts()
    statusCounts.value = apiStatusCounts
  } catch (error) {
    console.error('Failed to fetch status counts from API, using alternative approach:', error)
    // Fallback: fetch all rooms and calculate stats client-side
    try {
      await roomsStore.fetchRooms(1, 200) // Fetch more rooms for accurate calculation
      statusCounts.value = computedStatusCounts.value
      console.log('Using calculated status counts from loaded rooms:', statusCounts.value)
    } catch (fetchError) {
      console.error('Failed to fetch rooms for status calculation:', fetchError)
      // Final fallback to zero counts
      statusCounts.value = {
        available: 0,
        occupied: 0,
        cleaning: 0,
  ...
  window.addEventListener('rooms-changed', handleRoomStatusUpdate)
})

// Cleanup listeners on unmount
onUnmounted(() => {
  window.removeEventListener('room-status-updated', handleRoomStatusUpdate)
  window.removeEventListener('rooms-changed', handleRoomStatusUpdate)
})
</script>
